...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jai/Desktop/Neurotech/PsychoPy/y_z_lastrun.py',
...
y_ = visual.TextStim(win=win, name='y_',
    text='Y',
    font='Open Sans',
...
m__rComponents = [left_top_circle, left_bottom_circle, center_top_circle, center_bottom_circle, right_top_circle, right_bottom_circle, y_, z, textbox, back, back_text]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *y_* updates
    if y_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        y_.frameNStart = frameN  # exact frame index
        y_.tStart = t  # local t and not account for scr refresh
        y_.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(y_, 'tStartRefresh')  # time at next scr refresh
        y_.setAutoDraw(True)
    if y_.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > y_.tStartRefresh + 20.0-frameTolerance:
            # keep track of stop time/frame for later
            y_.tStop = t  # not accounting for scr refresh
            y_.frameNStop = frameN  # exact frame index
            win.timeOnFlip(y_, 'tStopRefresh')  # time at next scr refresh
            y_.setAutoDraw(False)
...
thisExp.addData('y_.started', y_.tStartRefresh)
thisExp.addData('y_.stopped', y_.tStopRefresh)
