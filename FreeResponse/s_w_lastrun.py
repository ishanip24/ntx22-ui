...
expName = 's_w'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jai/Desktop/Neurotech/PsychoPy/s_w_lastrun.py',
...
s = visual.TextStim(win=win, name='s',
    ...
    languageStyle='LTR',
    depth=-6.0);
t_ = visual.TextStim(win=win, name='t_',
    text='T\n\n',
    font='Open Sans',
    pos=(0, .045), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-7.0);
...
w = visual.TextStim(win=win, name='w',
    ...
    languageStyle='LTR',
    depth=-10.0);
x_ = visual.TextStim(win=win, name='x_',
    text='X',
    font='Open Sans',
    pos=(.6, -.15), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
back_text = visual.TextStim(win=win, name='back_text',
    text='back',
    font='Open Sans',
    pos=(-.6, .4), height=0.035, wrapWidth=None, ori=0.0, 
...
m__rComponents = [left_top_circle, left_bottom_circle, center_top_circle, center_bottom_circle, right_top_circle, right_bottom_circle, s, t_, u, v, w, x_, textbox, back, back_text]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *t_* updates
    if t_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        t_.frameNStart = frameN  # exact frame index
        t_.tStart = t  # local t and not account for scr refresh
        t_.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(t_, 'tStartRefresh')  # time at next scr refresh
        t_.setAutoDraw(True)
    if t_.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > t_.tStartRefresh + 20.0-frameTolerance:
            # keep track of stop time/frame for later
            t_.tStop = t  # not accounting for scr refresh
            t_.frameNStop = frameN  # exact frame index
            win.timeOnFlip(t_, 'tStopRefresh')  # time at next scr refresh
            t_.setAutoDraw(False)
    ...
    # *x_* updates
    if x_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        x_.frameNStart = frameN  # exact frame index
        x_.tStart = t  # local t and not account for scr refresh
        x_.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(x_, 'tStartRefresh')  # time at next scr refresh
        x_.setAutoDraw(True)
    if x_.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > x_.tStartRefresh + 20.0-frameTolerance:
            # keep track of stop time/frame for later
            x_.tStop = t  # not accounting for scr refresh
            x_.frameNStop = frameN  # exact frame index
            win.timeOnFlip(x_, 'tStopRefresh')  # time at next scr refresh
            x_.setAutoDraw(False)
...
thisExp.addData('t_.started', t_.tStartRefresh)
thisExp.addData('t_.stopped', t_.tStopRefresh)
...
thisExp.addData('x_.started', x_.tStartRefresh)
thisExp.addData('x_.stopped', x_.tStopRefresh)
