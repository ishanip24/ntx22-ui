...
expName = 'Numbers_v2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jai/Desktop/Covid Spreader/ntx22-ui/Numbers/Numbers_v2_lastrun.py',
...
textbox = visual.TextBox2(
     ...
     padding=0.0,
     anchor='center',
     fillColor='0.9451, 0.9451, 1.0000', borderColor='1.0000, 0.9608, 0.8824',
...
Choose_Number_Components = [button_1, button_05, button_08, button_02, button_06, button_03, button_07, button_00, button_04, button_09, textbox, back, submit]
...
while continueRoutine:
    ...
    if button_1.status == STARTED:  # only update if drawing
        button_1.setOpacity(sin(t*4), log=False)
    ...
    if button_05.status == STARTED:  # only update if drawing
        button_05.setOpacity(sin(t*16), log=False)
    ...
    if button_08.status == STARTED:  # only update if drawing
        button_08.setOpacity(sin(t*25), log=False)
    ...
    if button_02.status == STARTED:  # only update if drawing
        button_02.setOpacity(sin(t*7), log=False)
    ...
    if button_06.status == STARTED:  # only update if drawing
        button_06.setOpacity(sin(t*19), log=False)
    ...
    if button_03.status == STARTED:  # only update if drawing
        button_03.setOpacity(sin(t*10), log=False)
    ...
    if button_07.status == STARTED:  # only update if drawing
        button_07.setOpacity(sin(t*22), log=False)
    ...
    if button_00.status == STARTED:  # only update if drawing
        button_00.setOpacity(sin(t*31), log=False)
    ...
    if button_04.status == STARTED:  # only update if drawing
        button_04.setOpacity(sin(t*13), log=False)
    ...
    if button_09.status == STARTED:  # only update if drawing
        button_09.setOpacity(sin(t*28), log=False)
