...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/ishani/Desktop/school/neurotech/Home/Home Page_lastrun.py',
...
Backspace = visual.ImageStim(
    win=win,
    name='Backspace', 
    image='C:/Users/email/Downloads/backspace!.png', mask=None,
    ori=0.0, pos=(-.3, 0), size=[.2,.1],
...
# keep track of which components have finished
trialComponents = [textbox, Backspace, Yes, No, Number, MCQ, FRQ, mouse]
...
while continueRoutine:
    ...
    # *Backspace* updates
    if Backspace.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        Backspace.frameNStart = frameN  # exact frame index
        Backspace.tStart = t  # local t and not account for scr refresh
        Backspace.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(Backspace, 'tStartRefresh')  # time at next scr refresh
        Backspace.setAutoDraw(True)
    if Backspace.status == STARTED:  # only update if drawing
        Backspace.setOpacity(sin(5*t), log=False)
    ...
    if Yes.status == STARTED:  # only update if drawing
        Yes.setOpacity(sin(7*t), log=False)
    ...
    if No.status == STARTED:  # only update if drawing
        No.setOpacity(sin(9*t), log=False)
    ...
    if Number.status == STARTED:  # only update if drawing
        Number.setOpacity(sin(11*t), log=False)
    ...
    if MCQ.status == STARTED:  # only update if drawing
        MCQ.setOpacity(sin(13*t), log=False)
    ...
    if FRQ.status == STARTED:  # only update if drawing
        FRQ.setOpacity(sin(15*t), log=False)
...
thisExp.addData('Backspace.started', Backspace.tStartRefresh)
thisExp.addData('Backspace.stopped', Backspace.tStopRefresh)
