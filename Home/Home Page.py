...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\email\\Documents\\UC DAVIS\\Neurotech\\Home Page.py',
...
textbox = visual.TextBox2(
     win, text='Type Here', font='Open Sans',
...
Delete = visual.ButtonStim(win, 
    text='Delete', font='Arvo',
    pos=(-.3, 0),
    letterHeight=0.05,
    size=[.2,.1], borderWidth=2.0,
    fillColor='darkgrey', borderColor='black',
...
# keep track of which components have finished
trialComponents = [MCQ, FRQ, textbox, mouse, Number, Yes, No, Delete]
...
while continueRoutine:
    ...
    # *Delete* updates
    if Delete.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
    ...
    if Delete.status == STARTED:
        # check whether Delete has been pressed
        if Delete.isClicked:
            if not Delete.wasClicked:
                Delete.timesOn.append(Delete.buttonClock.getTime()) # store time of first click
                Delete.timesOff.append(Delete.buttonClock.getTime()) # store time clicked until
            else:
                Delete.timesOff[-1] = Delete.buttonClock.getTime() # update time clicked until
            if not Delete.wasClicked:
                None
            Delete.wasClicked = True  # if Delete is still clicked next frame, it is not a new click
        else:
            Delete.wasClicked = False  # if Delete is clicked next frame, it is a new click
    else:
        Delete.wasClicked = False  # if Delete is clicked next frame, it is a new click
...
thisExp.addData('Delete.numClicks', Delete.numClicks)
if Delete.numClicks:
   thisExp.addData('Delete.timesOn', Delete.timesOn)
   thisExp.addData('Delete.timesOff', Delete.timesOff)
else:
   thisExp.addData('Delete.timesOn', "")
   thisExp.addData('Delete.timesOff', "")
